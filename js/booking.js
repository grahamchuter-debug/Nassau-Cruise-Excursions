/**
 * Nassau request-to-book form (Phase 18D).
 * Guest model: adult_child — Adults + Children (4–12). Ages 0–3 not selectable.
 * Server-side pricing is authoritative — client total is for review only.
 */
(function () {
  const form = document.getElementById("nas-booking-form");
  if (!form || !window.NAS_COMMERCIAL) return;

  const cfg = window.NAS_COMMERCIAL;
  ...
  if (live) {
    ...
    if (statusBody) {
      statusBody.textContent =
        "Payment receives your request — it does not confirm the excursion. Confirmation is emailed after we arrange your places. If we cannot confirm, the payment is refunded in full. Live card payments are not enabled yet.";
    }
    if (footerNote) {
      footerNote.innerHTML =
  ...
  function childCount() {
    return guestModel === "adult_child" || guestModel === "adult_child_infant" ? num("children", 0) : 0;
  }
  ...
  function refresh() {
    ...
    const childrenCell = document.getElementById("rev-children");
    if (childrenRow) childrenRow.hidden = false;
    if (childrenCell) set("rev-children", children + " × $" + (childRate == null ? 0 : childRate));
    const totalGuests = adults + children;
    const total = adults * adultRate + (childRate == null ? 0 : children * childRate);
    if (totalGuests > max) set("rev-total", "Too many guests (max " + max + ")");
    else set("rev-total", "USD $" + total);
    return { adults, children, totalGuests, totalCents: Math.round(total * 100) };
  }
  ...
  function sessionId() {
    ...
    return "nas-" + Date.now() + "-" + Math.random().toString(36).slice(2, 10);
  }
  ...
  form.addEventListener("submit", async function (event) {
    ...
    const totalCents = Math.round(
      adults * adultRate * 100 + (childRate == null ? 0 : children * childRate * 100),
    );
    const payload = {
