/**
 * Grenada request-to-book Worker (Willemstad Historic Walking Tour only).
 *
 ...
 */
import { nassauBookingCore } from "../../../shared/destinations/nassau";
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Nassau Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findNassauBookingProduct(body.productId);
  ...
  const reference = createBookingReference(nassauBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(nassauBookingCore);
