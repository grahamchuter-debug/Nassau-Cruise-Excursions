...
export function supplierRequestEmail(input: {
  ...
  summaryRows.push(
    ...
    { label: "Booking status", value: "requested" },
  );
  ...
  const notes = input.product.supplierReferenceNotes ?? [];
  const internalSupply = notes
    .filter((note) => /INTERNAL SUPPLY|SEG|shoreexcursionsgroup|CASLJUNSOUVAN|CASLSAIL|canahike/i.test(note))
    .join(" | ");
  if (internalSupply) {
    summaryRows.push({ label: "INTERNAL supply reference", value: internalSupply });
  }
  const fulfilmentNote = notes.find((note) => /SEG_MANUAL|fulfilment_mode/i.test(note));
  if (fulfilmentNote && /SEG_MANUAL/i.test(fulfilmentNote)) {
    summaryRows.push({ label: "Source fulfilment", value: "SEG_MANUAL" });
  }
  const codeNote = notes.find((note) => /INTERNAL CODE:\s*(\S+)/i.test(note));
  if (codeNote) {
    const match = codeNote.match(/INTERNAL CODE:\s*(\S+)/i);
    if (match?.[1]) {
      summaryRows.push({ label: "Internal source code", value: match[1] });
    }
  }
